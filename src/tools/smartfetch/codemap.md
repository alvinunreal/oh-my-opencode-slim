...
- **Cache keyed by fetch shape:** `cache.ts` includes URL, `extract_main`, `prefer_llms_txt`, output `format`, and `save_binary`, so the tool does not incorrectly reuse a fetch result across materially different render modes.
...
- `secondary-model.ts` depends on the OpenCode plugin client (`PluginInput['client']`) to spawn an isolated helper session and read configured fallback models from `~/.config/opencode/opencode.json`.
