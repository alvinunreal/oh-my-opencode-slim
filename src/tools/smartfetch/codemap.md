...
- `secondary-model.ts` depends on the OpenCode plugin client (`PluginInput['client']`) to spawn an isolated helper session. The secondary-model chain and helper agent are resolved purely in memory at plugin construction: dedicated `webfetch` models, then the host's `small_model`, then the `explorer` / `librarian` agent models; the helper agent is selected from the registered, non-orchestrator agents after disabled-agent filtering. No config files are re-read on the webfetch hot path — see `config-read-guard.test.ts`.
