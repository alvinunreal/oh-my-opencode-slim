...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'oh-my-opencode-ultraslim', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[oh-my-opencode-ultraslim] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[oh-my-opencode-ultraslim] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[oh-my-opencode-ultraslim] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[oh-my-opencode-ultraslim] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
