...
describe('runDoctorCheck', () => {
  ...
  test('valid project config returns ok', () => {
    ...
    fs.writeFileSync(
      path.join(configDir, 'oh-my-opencode-ultraslim.jsonc'),
      `{
  ...
  test('config deleted after discovery returns read error with path', () => {
    const projectDir = path.join(tempDir, 'project');
    const configDir = path.join(projectDir, '.opencode');
    const configPath = path.join(configDir, 'oh-my-opencode-ultraslim.json');
    fs.mkdirSync(configDir, { recursive: true });
  ...
  test('empty config file returns invalid-json error', () => {
    ...
    const configDir = path.join(projectDir, '.opencode');
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(path.join(configDir, 'oh-my-opencode-ultraslim.json'), '');
