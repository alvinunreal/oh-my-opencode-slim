...
| `command-marker.test.ts` | Kit unit tests: pinned pattern sources, exact wrap shapes, wrap→parse→strip round-trips for both marker kinds, `$`-safety, whole-text anchoring, non-greedy closing-tag recovery, and the `trimArgs` raw-vs-trimmed semantics. |
