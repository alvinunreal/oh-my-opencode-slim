...
| `interview-bridge.ts` | v2-only `/interview` marker command, trailing-message context bridge, v2 interview runtime, and per-session transcript projections (raw-event retention gated on active interviews, lazy memoized projection via `runtime.messages()`; inactive/stale entries are FIFO-capped at 1024, active interviews are exempt from eviction — retention is ultimately bounded by the service's live interview set). |
