...
const TODO_HYGIENE_TRANSFORM_TIMEOUT_MS = 500;
...
export function createTodoContinuationHook(
  ...
  async function consumePendingReminderForTransform(
    sessionID: string,
  ): Promise<string | null> {
    return withTimeout(
      hygiene.consumePendingReminder(sessionID),
      TODO_HYGIENE_TRANSFORM_TIMEOUT_MS,
      null,
    );
  }
  ...
  async function handleMessagesTransformSafe(output: {
    ...
    if (
      ...
      const reminder = await consumePendingReminderForTransform(
