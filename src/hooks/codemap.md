...
- **Hook composition**: Hooks are composed directly in `src/plugin-composer.ts` (the Plugin return type is the registry — each hook point key maps to its handler). No separate HookRegistry module exists.
