...
export function syncDirectory(directory: string): void {
  try {
    ...
    // Best-effort directory-entry durability; unsupported filesystems may not
    // permit directory fsync, so this is not a power-loss durability guarantee.
  }
}
...
function createCandidate(lockDir: string): LockEntry {
  ...
  const candidate: Omit<LockEntry, 'dev' | 'ino'> = {
    kind: 'candidate',
    name,
...
function acquireOnce(
  ...
  const candidate = createCandidate(lockDir);
  const ownLease = candidateToLease(candidate);
  let publishedLease: LockEntry | undefined;
  try {
    ...
    try {
      ...
      // Preserve the acquisition error if best-effort cleanup also fails.
    }
    throw error;
  }
}

type LeasePathStatus = 'present' | 'absent' | 'replaced';

function inspectLeasePath(lease: LockEntry): LeasePathStatus {
  try {
...
function retryPendingLeaseCleanup(): void {
  ...
  try {
    ...
    throw new MarketplaceLockfileError(
      `Unable to clean up a previously published marketplace lease: ${String(error)}`,
    );
  }
}
...
function createLease(lockDir: string, leaseEntry: LockEntry): MarketplaceLease {
  ...
  const assertCurrent = (): void => {
    ...
    if (
      ...
      inspectLeasePath(leaseEntry) !== 'present'
    ) {
      throw lost();
    }
  };
  ...
  const release = (): void => {
    if (status !== 'owned') throw lost();
    if (inspectLeasePath(leaseEntry) !== 'present') throw lost();
    try {
      ...
      try {
        ...
        if (inspectionError instanceof MarketplaceLockOwnershipError) {
...
function acquireMarketplaceLeaseInternal(
  ...
  while (Date.now() <= deadline) {
    const lease = acquireOnce(paths.lockDir, options);
