    ...
    withMarketplaceLease(
      ...
      (lease) => {
        ...
        lease.commit(() => {
          ...
          if (packageExists) fs.renameSync(packagePath, quarantinePath);
  ...
  private mutatePackage(
    ...
    return withMarketplaceLease(
      ...
      (lease) => {
        ...
        if (current) {
          let existing: StoredMarketplacePackage | undefined;
          try {
            existing = this.loadLockedPackage(lockfile, bundle.manifest.id);
          } catch (error) {
          ...
          if (
            ...
            damagedCurrent &&
            (current.manifestVersion !== bundle.manifest.version ||
              current.manifestSchemaVersion !== bundle.manifest.schemaVersion ||
              current.digest.algorithm !== 'sha256' ||
              current.digest.domain !==
            ...
            throw new MarketplaceConflictError(
              `${bundle.manifest.id}@${bundle.manifest.version} is locked with a different identity or digest`,
            );
          }
          if (
            mode === 'update' &&
        ...
        try {
          return lease.commit(() => {
            ...
            try {
              ...
              if (repairBackup) {
                try {
                  fs.rmSync(repairBackup, { recursive: true, force: true });
                } catch {
                  // The new lock entry is authoritative; reconciliation removes
                  // an abandoned repair backup on the next operation.
                }
              }
