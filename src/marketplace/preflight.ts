...
const V2McpTimeoutSchema = z.object({
  startup: PositiveTimeoutSchema.optional(),
  request: PositiveTimeoutSchema.optional(),
});
const V2McpDefinitionSchema = z.discriminatedUnion('type', [
  McpLocalSchema.omit({ enabled: true, timeout: true }).extend({
    disabled: z.boolean().optional(),
    timeout: V2McpTimeoutSchema.optional(),
  }),
  McpRemoteSchema.omit({ enabled: true, timeout: true }).extend({
    disabled: z.boolean().optional(),
    timeout: V2McpTimeoutSchema.optional(),
  }),
]);
  ...
  if (hostFlavor === 'v2') {
    ...
    return Object.fromEntries(
      Object.entries(servers).filter(
        ([, definition]) =>
          V2McpDefinitionSchema.safeParse(definition).success &&
...
export function discoverPreflightMcps(
  ...
  for (const [name, definition] of Object.entries(
    ...
    if (
      disabled.has(name) ||
      (hostFlavor !== 'v2' && !isValidMcpDefinition(definition))
    )
      continue;
    names.add(name);
  }
...
/** Desired v2 MCP names are read from disk, not the frozen live host snapshot.
 * Slim builtins are not represented in opencode.json, so keep only the ones
 * actually present at registry finalization. Other host-provided transforms
 * cannot be predicted from disk and are intentionally not assumed here. */
