...
describe('OpenCode preflight discovery parity', () => {
  ...
  test('v2 reads only enabled, valid mcp.servers definitions', () => {
    const root = mkdtempSync(join(tmpdir(), 'marketplace-preflight-'));
    try {
      process.env.XDG_CONFIG_HOME = join(root, 'config');
      process.env.OPENCODE_CONFIG_CONTENT = JSON.stringify({
        mcp: {
          timeout: 12000,
          config: { fake: { type: 'remote', url: 'https://fake.test' } },
          servers: {
            custom_docs: {
              type: 'remote',
              url: 'https://docs.test',
      ...
      RuntimeConfig.reset(root);
      const names = discoverPreflightMcps(
        RuntimeConfig.init(root, { disabled_mcps: ['gh_grep'] }),
        root,
        'v2',
      );
      expect(names).toContain('custom_docs');
