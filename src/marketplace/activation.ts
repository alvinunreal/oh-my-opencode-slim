...
function ambiguousMcpNamespaces(
  ...
  // Match the native action names used by the agent permission projection.
  const namespaces = new Map(
    [...available].map((name) => [name, name.replace(/[^a-zA-Z0-9_-]/g, '_')]),
  );
  for (const name of required) {
    const namespace = namespaces.get(name);
    if (namespace === undefined) continue;
    for (const [other, otherNamespace] of namespaces) {
      if (
        other !== name &&
        (otherNamespace === namespace ||
          otherNamespace.startsWith(`${namespace}_`) ||
          namespace.startsWith(`${otherNamespace}_`))
      ) {
        conflicts.add(`${name} and ${other}`);
      }
    }
  }
...
export function resolveMarketplaceActivation(
  ...
  for (const packageId of [...ids].sort()) {
    ...
    const ambiguousMcps = ambiguousMcpNamespaces(pkg.manifest.mcps, mcps);
    if (ambiguousMcps.length > 0) {
      diagnostics.push(
        diagnostic(
          id,
          'ambiguous-mcp-namespace',
          `${id} is disabled: ambiguous MCP action namespace between ${ambiguousMcps.join(', ')}`,
        ),
      );
      continue;
    }
