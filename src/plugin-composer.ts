...
export function createPluginComposer(): PluginComposer {
  return {
    async compose(ctx: Parameters<Plugin>[0]) {
      ...
      let taskSessionManagerHook: ReturnType<
        typeof createTaskSessionManagerHook
      >;
      ...
      try {
        ...
        for (const agentDef of agentDefs) {
          if (agentDef._modelArray?.length) {
            ...
            runtimeChains[agentDef.name] = agentDef._modelArray.map(
              (m) => m.id,
            );
          }
        }
