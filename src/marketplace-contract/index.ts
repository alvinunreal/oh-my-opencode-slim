...
} from '../marketplace/canonical.js';
import { satisfiesPluginCompatibility } from '../marketplace/compatibility.js';
import { normalizeMarketplacePackageId } from '../marketplace/ids.js';
import { renderMarketplaceAutoDelegationBlock } from '../marketplace/routing.js';
import {
...
function validateRegistryEntries(
  ...
  for (let position = 0; position < entries.length; position += 1) {
    ...
    if (
      previous &&
      valid(previous.version) !== null &&
      valid(entry.version) !== null &&
...
export function validateMarketplaceRegistryEntry(
  ...
  const validatedEntry = MarketplaceRegistryEntrySchema.safeParse(entry);
  if (!validatedEntry.success) {
    throw new Error(
      `Invalid marketplace registry entry: ${validatedEntry.error.message}`,
    );
  }
  if (
    validatedEntry.data.artifactPath !==
      registryArtifactPath(
        validatedEntry.data.id,
        validatedEntry.data.version,
      ) ||
    validatedEntry.data.summary.id !== validatedEntry.data.id ||
    validatedEntry.data.summary.version !== validatedEntry.data.version
  ) {
    throw new Error('Marketplace registry entry identity is inconsistent');
  }
  ...
  if (
    manifest.id !== validatedEntry.data.id ||
    manifest.version !== validatedEntry.data.version
  ) {
    throw new Error(
  ...
  if (
    ...
    canonicalizeMarketplaceValue(validatedEntry.data.summary)
  ) {
    throw new Error(
  ...
  if (validatedEntry.data.digest.value !== digest) {
    throw new Error(
...
export function validateMarketplaceRegistryEntryV3(
  ...
  const validatedEntry = MarketplaceRegistryEntryV3Schema.safeParse(entry);
  if (!validatedEntry.success) {
    throw new Error(
      `Invalid marketplace v3 registry entry: ${validatedEntry.error.message}`,
    );
  }
  if (
    validatedEntry.data.artifactPath !==
    ...
    throw new Error('Marketplace v3 registry entry identity is inconsistent');
  }
...
export function parseMarketplaceRegistrySelector(
  ...
  try {
    normalizedId = normalizeMarketplacePackageId(id);
  } catch {
    normalizedId = '';
  }
  const idResult = MarketplacePackageIdSchema.safeParse(normalizedId);
  if (
    !idResult.success ||
    (version !== undefined &&
