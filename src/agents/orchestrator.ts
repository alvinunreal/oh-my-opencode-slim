...
- Use \`task_steer\` when a running child appears stuck, loops, or drifts from its objective and should be redirected rather than discarded: it interrupts the current generation and relaunches the same session with your steering instruction, preserving accumulated context. Prefer steering over cancel-and-redispatch when the child's accumulated context is still valuable.
