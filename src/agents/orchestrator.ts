...
- If a delegated task appears stuck, loops, or drifts from its objective, do not keep waiting and do not spawn a fresh session: resume the same retained session with \`task_revive\` and a corrective prompt. It interrupts a running generation when needed and relaunches that session, preserving accumulated context. Never use it as a status check, and never claim the new prompt was seen until the child produces a result.
- Always delegate with \`task(..., background: true)\`; never launch a foreground (blocking) \`task\` — foreground children sit outside the Background Job Board supervise/recover workflow.
