...
import { discoverDesiredV2Mcps } from '../marketplace/preflight';
...
import { adaptPermissions, delegationVocabulary } from '../v2/adapters';
import {
...
export interface ResolvedAgentRegistry {
  ...
  >;
  /** Configured candidates before host-selected model projection. */
  readonly configuredModelChains: Readonly<Record<string, readonly string[]>>;
  ...
  >;
  readonly v2PermissionPolicies: Readonly<Record<string, PermissionPolicy>>;
...
export function resolveDesiredMarketplaceLiveFromDisk(
  ...
  try {
    const desiredV2Mcps =
      liveContext?.hostFlavor === 'v2'
        ? discoverDesiredV2Mcps(
            runtime,
            projectDirectory,
            liveContext.availableMcpNames,
          )
        : undefined;
...
function applyModelInheritance(
  ...
  if (override?.inheritModelFrom === 'session') {
    // An owner-provided array is the fallback chain; only a lower-layer
    // marketplace chain is replaced by inheritance without an owner model.
    if (override.model === undefined) delete agent._modelArray;
...
export function applyModelInheritanceToConfig(
  ...
  for (const agentName of Object.keys(configAgent)) {
    ...
    const preserveVariant = override.variant !== undefined || hasHostVariant;
    if (override.inheritModelFrom === 'session') {
    ...
    // Explicit agent-level and host variants win; an inline chain-head
    // variant does not. Mirrors the agent-layer cleanup above.
    if (!preserveVariant) {
...
function marketplacePermissionCeilings(
  ...
  capabilities: ReturnType<typeof marketplaceCapabilityCeilings>,
  availableMcpNames: readonly string[],
  ...
  const skillMap = normalizePermission(skillPermission);
  const skillEffects = Object.fromEntries(
    capabilities.skills.flatMap((skill) => {
      const value =
        typeof skillPermission === 'string'
          ? skillPermission
          : (skillMap[skill] ?? skillMap['*']);
      return value === 'deny' || value === 'ask' ? [[skill, value]] : [];
    }),
  ) as Record<string, 'ask' | 'deny'>;
  const admittedMcps = capabilities.mcps.filter((name) =>
    availableMcpNames.includes(name),
  );
  const mcpNamespaces = admittedMcps.map(
    (name) => `${name.replace(/[^a-zA-Z0-9_-]/g, '_')}_*`,
  );
  const mcpEffects = Object.fromEntries(
    mcpNamespaces.flatMap((namespace) => {
      const effect = permission[namespace];
      return effect === 'deny' || effect === 'ask' ? [[namespace, effect]] : [];
    }),
  ) as Record<string, 'ask' | 'deny'>;
  return {
    actions,
...
export function createAgents(
  ...
  const customAgentNames = uniqueNames([
    ...
    ...Object.keys(mergedAgents).filter((name) => !isKnownAgentName(name)),
  ])
      ...
      const agent: AgentDefinition = role
        ...
        : {
            ...
            config: {
              ...
              prompt: activated.manifest.prompt,
            },
          };
      agent.name = name;
  ...
  const routingGuidance = buildRoutingGuidance(runtime, allSubAgents);
  const builtinRoleNames = new Set<string>(SUPPORTED_SPECIALIST_ROLES);
  const builtinRoutingGuidance = new Map(
    [...routingGuidance].filter(([name]) => builtinRoleNames.has(name)),
  );
  const nonBuiltinRoutingGuidance = new Map(
    [...routingGuidance].filter(([name]) => !builtinRoleNames.has(name)),
  );
  ...
  const orchestrator = createOrchestratorAgent(
    ...
    options?.hostFlavor,
    buildRoutingEntriesFromAgents(
      allSubAgents,
      nonBuiltinRoutingGuidance,
      marketplace,
    ),
  ...
  const projectRoutingGuidance = Array.from(builtinRoutingGuidance.values());
  if (projectRoutingGuidance.length > 0) {
    updatedPrompt = `${updatedPrompt}\n\n# Project-specific routing guidance\n\n${projectRoutingGuidance.join(
      '\n\n',
    )}`;
  }
...
function buildRegistryFromMarketplace(
  ...
  const v2PermissionPolicies: Record<string, PermissionPolicy> = {};
  ...
  // Preserve the configured candidate order before projecting a host's
  // current model selection over the registry's finalized model surfaces.
  const configuredModelChains = Object.fromEntries(
  ...
  for (const agent of agents) {
    ...
    const combinedInheritance = runtime.combinedModelInheritanceSource(name);
    ...
    let resolvedMarketplaceCapabilities:
      | ReturnType<typeof marketplaceCapabilityCeilings>
      | undefined;
    if (marketplaceEntry) {
  ...
  for (const [name, models] of Object.entries(modelArrays)) {
    ...
    if (
      ...
      models.length > 0 &&
      !runtime.combinedModelInheritanceSource(name) &&
      getOverrideFromAgents(runtime.agents(), name)?.inheritModelFrom ===
        undefined
    ) {
  ...
  for (const agent of agents) {
    ...
    if (agent.displayName) {
      ...
      if (configuredModelChains[agent.name]) {
        configuredModelChains[displayName] = configuredModelChains[agent.name];
      }
      mcpLists[displayName] = mcpLists[agent.name];
      skillPermissions[displayName] = skillPermissions[agent.name];
      v2PermissionPolicies[displayName] = v2PermissionPolicies[agent.name];
  ...
  for (const [alias, canonical] of Object.entries(AGENT_ALIASES)) {
    if (canonicalIdByRuntimeName[canonical]) {
      ...
      v2PermissionPolicies[alias] = v2PermissionPolicies[canonical];
    }
  }
  ...
  return deepFreeze({
    ...
    configuredModelChains: Object.freeze(
      Object.fromEntries(
        Object.entries(configuredModelChains).map(([name, chain]) => [
          name,
          Object.freeze([...chain]),
        ]),
      ),
    ...
    v2PermissionPolicies: Object.freeze(v2PermissionPolicies),
