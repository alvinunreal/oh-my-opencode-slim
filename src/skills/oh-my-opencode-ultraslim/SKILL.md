...
description: Configure and improve oh-my-opencode-ultraslim for the current user. Use when users want to tune agents, models, prompts, custom agents, skills, MCPs, presets, or plugin behavior. Also use when recurring workflow friction suggests a safe config or prompt improvement.
---

# oh-my-opencode-ultraslim Configuration Skill

You help users configure, customize, and safely improve their
oh-my-opencode-ultraslim setup.
...
oh-my-opencode-ultraslim is configured through a plugin config file, usually:
...
```text
~/.config/opencode/oh-my-opencode-ultraslim.jsonc
```
...
| `~/.config/opencode/oh-my-opencode-ultraslim.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/opencode/oh-my-opencode-ultraslim.json` | User plugin config generated by installer |
| `<project>/.opencode/oh-my-opencode-ultraslim.json` | Project-local plugin overrides checked before user config |
| `~/.config/opencode/oh-my-opencode-ultraslim/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/opencode/oh-my-opencode-ultraslim/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
| `~/.config/opencode/oh-my-opencode-ultraslim/{preset}/{agent}.md` | Preset-specific full prompt replacement |
| `~/.config/opencode/oh-my-opencode-ultraslim/{preset}/{agent}_append.md` | Preset-specific append-only prompt tuning |
...
1. If a `preset` is active, check
   `~/.config/opencode/oh-my-opencode-ultraslim/{preset}/` first.
2. Fall back to `~/.config/opencode/oh-my-opencode-ultraslim/`.
...
- Built-in agent `prompt` and `orchestratorPrompt` fields are **not** supported
  in `oh-my-opencode-ultraslim.json[c]`; use markdown prompt override files instead.
...
```text
~/.config/opencode/oh-my-opencode-ultraslim/orchestrator_append.md
```

or for only the `openai` preset:

```text
~/.config/opencode/oh-my-opencode-ultraslim/openai/orchestrator_append.md
```
...
```text
~/.config/opencode/oh-my-opencode-ultraslim/orchestrator.md
```
...
1. **Inspect current setup**
   - Read the existing oh-my-opencode-ultraslim config file.
...
~/.config/opencode/oh-my-opencode-ultraslim/orchestrator_append.md. Apply it?
```
...
public API changes to it. Apply to your oh-my-opencode-ultraslim config?
```
