...
| `src/hooks/loop-command/` | `/loop` user-entry adapter: directs the parent/orchestrator to load the `loop-engineering` bounded delivery protocol for a prepared unit, preserving user context and the delivery handoff contract. | [View Map](src/hooks/loop-command/codemap.md) |
