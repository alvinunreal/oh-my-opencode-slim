...
```bash
bunx oh-my-opencode-slim install --kimi=yes --openai=yes --antigravity=yes --opencode-free=yes --opencode-free-model=auto
```
...
4. "Do you want to use **OpenCode free models**?" *(Refreshes and selects from free `opencode/*` models)*
...
```bash
bunx oh-my-opencode-slim@latest install --no-tui --kimi=<yes|no> --openai=<yes|no> --antigravity=<yes|no> --opencode-free=<yes|no> --opencode-free-model=<id|auto> --tmux=<yes|no> --skills=<yes|no>
```
...
# OpenAI only
bunx oh-my-opencode-slim@latest install --no-tui --kimi=no --openai=yes --antigravity=no --opencode-free=no --tmux=no --skills=yes

# OpenCode free models only (auto-select)
bunx oh-my-opencode-slim@latest install --no-tui --kimi=no --openai=no --antigravity=no --opencode-free=yes --opencode-free-model=auto --tmux=no --skills=yes

# OpenCode free models + OpenAI (manual primary model)
bunx oh-my-opencode-slim@latest install --no-tui --kimi=no --openai=yes --antigravity=no --opencode-free=yes --opencode-free-model=opencode/gpt-5-nano --tmux=no --skills=yes
```
